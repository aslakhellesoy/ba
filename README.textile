...
<pre><code>rake db:bootstrap (Say yes to blow away db and choose the "Empty" database template if you're new to Radiant)
...
In order for your users to be able to manage their own account, you need a login page. The Ba database 
migrations will create a Login page for you at /login with default content. You can change the content 
in the admin interface. Make sure you keep the same form and input fields. The login page has published
status by default.
