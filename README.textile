...
rake db:migrate:extensions radiant:extensions:update_all
</code></pre>
...
In addition to creating the HappeningPage, this will also create several sub pages:

<pre><code>
+-LoginPage           (Global login page)
+-HappeningPage       (The main page - may be at a deeper level)
  +-SignupPage        (The signup form)
  +-AttendancePage    (Add talk proposals, edit promotion codes)
</code></pre>

This will create some new parts with default content. 

The "attendance" part has the content for [happening url]/attendance, which is where people will sign up for the happening.
The "signup_confirmation_email" part is for signup confirmation. More on that below.
...
In the body you can use any r:ba:email tag to add personalised content (NOT YET IMPLEMENTED).

h3. Sending mass emails (NOT YET IMPLEMENTED)

If you have configured ActionMailer as described above, you can also send mass emails to combinations of attendees
and speakers of a happening. This is done from the admin interface.
...
Ba comes with a script that can import users into the SiteUser table. You first have to create a CSV file that looks something
like the following:
...
The first line must contain column names of the SiteUser table, and the remaining rows the values.
...
In order for your users to be able to manage their own account, you need a login page. The Ba database migrations will create 
a Login page for you at /login with default content. You can change the content in the admin interface. Make sure you keep
the same form and input fields. The login page have published status by default.
